...
import type { HrGroup, HrCategory, Company } from '@/lib/types';

const ROUTE = '/hr/groups';
const MAX_LEVEL = 5;
...
export default function HrGroupsPage() {
  ...
  const save = async (mode: SaveMode = 'saveClose') => {
    ...
    try {
      ...
      if (editing) {
        ...
        // editable fields.
        saved = await api.patch<HrGroup>(`/hr-groups/${editing.id}`, {
          name: form.name.trim(),
        ...
        toast.success('Manpower group updated.');
      } else {
        saved = await api.post<HrGroup>('/hr-groups', {
        ...
        toast.success('Manpower group created.');
      }
      await refetch();
      if (mode === 'saveNew') {
  ...
  const remove = async (g: HrGroup) => {
    const ok = await confirm({
      title: 'Delete manpower group',
    ...
    if (!ok) return;
    try {
      await api.delete(`/hr-groups/${g.id}`);
      toast.success('Manpower group deleted.');
      refetch();
    } catch (e) {
  ...
  const availabilityText = (g: HrGroup) =>
    g.companyIds.map((id) => companyNameById.get(id) ?? `#${id}`).join(', ');

  const columns: Column<HrGroup>[] = [
  ...
  return (
    <div className="mx-auto flex h-full max-w-7xl flex-col">
      <PageHeader
        title="Group Master"
        description="Multilayer manpower groups under a category (up to 5 levels), with auto-generated codes"
      ...
      <DataTable
        ...
        fillHeight
        onRefresh={refetch}
        searchPlaceholder="Search manpower groups..."
        rowClassName={(r) =>
          r.level === 1
        ...
        emptyMessage="No manpower groups found"
      />
      ...
      <Drawer
        ...
        onClose={closeDrawer}
        title={title}
        subtitle="Manpower group details"
        icon={<Layers className="h-5 w-5" />}
        footer={
          view ? (
            ...
            <div className="flex flex-col gap-1 sm:col-span-2">
              ...
              <p className="ml-6 text-xs text-slate-500 dark:text-slate-400">
                {form.subGroupApplicable
                  ? 'Designations cannot be added here — only sub-groups.'
                  : 'Leaf group — designations are created directly under it.'}
                {!canBeContainer &&
