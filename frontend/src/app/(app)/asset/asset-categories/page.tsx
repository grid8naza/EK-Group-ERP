...
const ROUTE = '/asset/asset-categories';
...
export default function AssetCategoriesPage() {
  ...
  const formFrom = (c: AssetCategory) => ({
    code: c.code,
    name: c.name,
  ...
  const openView = (c: AssetCategory) => {
    setEditing(c);
    setView(true);
    setForm(formFrom(c));
    setOpen(true);
  ...
  const save = async (mode: SaveMode = 'saveClose') => {
    ...
    setSaving(true);
    try {
      let saved: AssetCategory;
      if (editing) {
        saved = await api.patch<AssetCategory>(
          `/asset-categories/${editing.id}`,
          payload,
        );
        toast.success('Asset category updated.');
      } else {
        saved = await api.post<AssetCategory>('/asset-categories', payload);
  ...
  const toggleActive = (c: AssetCategory) =>
    guardEdit(c, async () => {
      try {
        await api.patch(`/asset-categories/${c.id}`, { isActive: !c.isActive });
        toast.success(
          c.isActive
            ? 'Asset category set inactive.'
            : 'Asset category set active.',
        );
        refetch();
      } catch (e) {
  ...
  const availabilityText = (c: AssetCategory) =>
  ...
  return (
    <div className="mx-auto flex h-full max-w-7xl flex-col">
      <PageHeader
        ...
        description="Categories for Assets — available to all or to selected companies"
      ...
      <Drawer
        ...
        onClose={closeDrawer}
        title={title}
        subtitle="Asset category details"
        icon={<Tag className="h-5 w-5" />}
        footer={
          view ? (
            ...
            <div className="flex flex-col gap-2 sm:col-span-2">
              ...
              {!form.allCompanies && (
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  {form.companyIds.length} selected — the asset category is
