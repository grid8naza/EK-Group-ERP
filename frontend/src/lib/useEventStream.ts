...
/**
 * Subscribe to a server-sent-events endpoint, and call `onEvent` per event.
 *
 ...
 * token. The alternative is a token in the query string, which lands in proxy
 * and server logs. So the SSE framing is parsed here (it is four lines of it)
 * and every request in the app keeps carrying its credentials the same way.
 *
 * Reconnects with exponential backoff, and calls `onReconnect` when a new stream
 * opens after a broken one — the caller refetches there, since anything that
 * happened while the connection was down was never delivered.
 *
 * `path` is captured on mount and never re-read: a component streams from one
 * endpoint for its life, and making it reactive would tear down a working
 * connection on any render that produced a fresh string.
 */
export function useEventStream(
  ...
  useEffect(() => {
    ...
    const connect = async () => {
      ...
      try {
        const res = await fetch(`${API_URL}${pathRef.current}`, {
          headers: {
