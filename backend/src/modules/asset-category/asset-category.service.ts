...
// An asset-category row with its company links, flattened to companyIds.
const withCompanies = {
...
export class AssetCategoryService {
  ...
  /**
   * Asset categories available in the active company: ones flagged for all
  ...
  async findAll(companyId: number | undefined, search?: string) {
    const scopeFilter: Prisma.AssetCategoryWhereInput = companyId
      ? {
  ...
  private async nextCategoryNumber(): Promise<number> {
    ...
    if (n == null) {
      throw new BadRequestException(
        `Maximum of ${MAX_CATEGORY} asset categories reached.`,
      );
    }
    return n;
  }
  ...
  async update(
    ...
    assertUnlocked(existing, 'asset category', 'editing');
  ...
  async remove(companyId: number | undefined, id: number) {
    const existing = await this.findOne(companyId, id);
    assertUnlocked(existing, 'asset category', 'deleting');
    try {
      ...
      if (
        ...
        throw new ConflictException(
          'This asset category still has groups under it. Remove those first.',
        );
      }
      throw e;
    }
