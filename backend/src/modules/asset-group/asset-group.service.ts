...
// Asset group rows are returned with their parent category, parent group, and
...
export class AssetGroupService {
  ...
  /**
   * Asset groups available in the active company, optionally narrowed to a
  ...
  async findAll(
    ...
    let primaryFilter: Prisma.AssetGroupWhereInput = {};
    if (opts.primaryGroupId) {
      const primary = await this.prisma.assetGroup.findUnique({
    ...
    const rows = await this.prisma.assetGroup.findMany({
      where: {
        AND: [
          scopeFilter,
          primaryFilter,
  ...
  async create(dto: CreateAssetGroupDto) {
    ...
    return this.withCodeRetry(async () => {
      ...
      const created = await this.prisma.assetGroup.create({
        data: {
          categoryId,
          parentGroupId,
          level,
  ...
  async update(companyId: number | undefined, id: number, dto: UpdateAssetGroupDto) {
    ...
    assertUnlocked(existing, 'asset group', 'editing');
  ...
  async remove(companyId: number | undefined, id: number) {
    ...
    assertUnlocked(existing, 'asset group', 'deleting');
    try {
      ...
      if (
        ...
        throw new ConflictException(
          'This asset group has sub-groups under it. Remove those first.',
        );
      }
      throw e;
    }
  ...
  private async nextGroupNumber(
    ...
    const sibs = await this.prisma.assetGroup.findMany({
